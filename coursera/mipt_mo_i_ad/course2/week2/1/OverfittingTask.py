...
#    * ответ: выраженная сезонность с пиком в середине года
# 1. Укажите один или два признака, от которых число прокатов скорее всего зависит линейно
#    * ответ: температура (temp, atemp) и тип погоды (weathersit)
...
X_numeric = df[['temp', 'atemp', 'hum', 'windspeed(mph)', 'windspeed(ms)', 'cnt']]
X_numeric.corr()
...
zip_coef_col = zip(reg.coef_, df_shuffled.columns)
list(zip_coef_col)
...
zip_coef_col = zip(lasso_regressor.coef_, df_shuffled.columns)
list(zip_coef_col)
...
zip_coef_col = zip(ridge_regressor.coef_, df_shuffled.columns)
list(zip_coef_col)
...
for alpha in alphas:
    ...
    lasso_regressor.fit(X, y)
    for i, coef in enumerate(lasso_regressor.coef_):
        coefs_lasso[row][i] = coef 
    ridge_regressor = Ridge(random_state = 3, alpha = alpha)
    ridge_regressor.fit(X, y)
    for i, coef in enumerate(ridge_regressor.coef_):
        coefs_ridge[row][i] = coef
    row+=1
...
#     * Ответ: обнулятся, т.к. вес регулярихатора станет превышать суммарное влияние признаков
...
#     * Ответ: Да, исключает (0 для windspeed(ms)); Ridge не исключает, но windspeed на изменения alpha вообще не реагирует. 
...
lassocv = LassoCV(random_state = 3, cv = 3, alphas = alphas)
lassocv.fit(X, y)
mses = []
used_alphas = []

for mse, alpha in zip(lassocv.mse_path_, lassocv.alphas_):
    mses.append(mse.mean())
    used_alphas.append(alpha)
    
plt.figure()
plt.plot(used_alphas, mses)
plt.xlabel("alpha")
plt.ylabel("mse")
plt.title("LassoCV")


print("Победил альфа: ", lassocv.alpha_)
list(zip(lassocv.coef_, df.columns))
...
for i in range(0, 3):
    plt.figure()
    print("argmin(alpha): ", used_alphas[np.argmin(lassocv.mse_path_[:, i])])
    plt.plot(used_alphas, lassocv.mse_path_[:, i])
    plt.xlabel("alpha")
    plt.ylabel("mse 1 столбец")
    plt.title("LassoCV")
...
#     * Ответ: top4: season, yr, atemp, weekday; Из "здравого смысла" следует, что только с увеличением температуры спрос возрастастает - по всей видимости, данная зависимость должна быть ограничена , т.к. в +50 на велике ездить не прикольно. Также с днем недели - чем ближе к выходным, тем выше спрос - что, в целом, логично, однако, тут не совсем честный численный признак (он вообще-то категориальный) и его рост ограничен. Аналогичное можно сказать про сезонный рост. С годами связь интереснее, т.к. спрос с одной стороны растет от года к году - это логично, однако количества лет недостаточно для того, чтобы уверенно моно было говорить о тенденции.*
...
#     * Ответ: top3: weathersit, hum, windspeed(mph). Очевидно, увеличение всех перечисленных признаков соотвествует степени "хреновости" погоды и логично ожидать уменьшения спроса на велосипеды.
...
#     * Ответ: windspeed(ms) - линейно зависимый признак, который можно смело выкинуть из модели, если учтен windspeed(mph)
